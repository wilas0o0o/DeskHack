...
describe 'postsコントローラーのテスト' do
  ...
  describe '自分の投稿詳細画面のテスト' do
    before do
      visit post_path(original_post)
    end

    context '表示内容の確認' do
      it 'URLが正しい' do
        expect(current_path).to eq '/posts/' + original_post.id.to_s
      end
      it 'ユーザー名が表示されている' do
        expect(page).to have_content original_post.user.name
      end
      it 'ユーザーIDが表示されている' do
        expect(page).to have_content original_post.user.username
      end
      it 'ユーザーリンクが正しい' do
        expect(page).to have_link '', href: user_path(original_post.user)
      end
      it '投稿画像が表示されている' do
        expect(page).to have_selector("img[src$='test.jpg']")
      end
      it '投稿のtextが表示されている' do
        expect(page).to have_content original_post.text
      end
      it '投稿のcaptionがリンクで表示されている' do
        expect(page).to have_link original_post.caption
      end
      it '投稿のsituationが表示されている' do
        expect(page).to have_content original_post.situation
      end
      ...
      it '投稿の編集リンクが表示されている' do
        expect(page).to have_link '', href: edit_post_path(original_post)
      end
    ...
    context '編集リンクのテスト' do
      it '編集画面に遷移できる' do
        ...
        expect(current_path).to eq '/posts/' + original_post.id.to_s + '/edit'
      end
    end
  ...
  describe '投稿編集画面のテスト' do
    before do
      visit edit_post_path(original_post)
    end
    ...
    context '表示内容の確認' do
      ...
      it 'text編集フォームが表示されている' do
        expect(page).to have_field 'post[text]', with: original_post.text
      end
      it 'situation編集フォームが表示されている' do
        expect(page).to have_field 'post[situation]', with: original_post.situation
      end
      it 'tag編集フォームが表示されている' do
        expect(page).to have_field 'post[caption]', with: original_post.caption
      end
    end

    context '編集成功のテスト' do
      before do
        @ex_post_text = original_post.text
        fill_in 'post[text]', with: Faker::Lorem.characters(number: 10)
        click_button '変更する'
      end

      it 'textが正しく更新される' do
        expect(original_post.reload.text).not_to eq @ex_post_text
      end
